...
export interface ExportStockModalProps {
  isOpen: boolean;
  onClose: () => void;
  exportStockText: string;
}

export function ExportStockModal({ isOpen, onClose, exportStockText }: ExportStockModalProps) {
      ...
      <div className="bg-white border-4 border-slate-900 w-full max-w-2xl shadow-[16px_16px_0px_0px_#0f172a] my-auto">
        <div className="p-6 border-b-4 border-slate-900 flex items-center justify-between bg-slate-50">
          <h3 className="text-xl font-black text-slate-900 flex items-center gap-2 uppercase tracking-widest">
            <FileText className="w-6 h-6 border-2 border-slate-900 bg-indigo-100 p-1 shadow-[2px_2px_0px_0px_#0f172a]" />{" "}
            Data Export Stok Barang
          </h3>
          <button
            onClick={onClose}
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            <button
              onClick={() => {
                ...
                setTimeout(() => setExportStockToast(false), 2000);
              }}
              className="flex-1 py-4 bg-indigo-600 hover:bg-indigo-700 border-2 border-slate-900 text-white font-black uppercase tracking-widest text-xs shadow-[4px_4px_0px_0px_#0f172a] active:shadow-none transition-all flex items-center justify-center gap-2"
            >
              {exportStockToast ? (
                <>
            ...
            <button
              onClick={() => {
                const blob = new Blob([exportStockText], {
                  type: "text/plain;charset=utf-8",
                });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                a.download = `Export_Stok_${new Date().toISOString().slice(0, 10)}.txt`;
                a.click();
